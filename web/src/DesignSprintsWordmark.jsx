...
/** SPRINT wordmark — inline so hover fill classes apply (matches homepage). */
export function DesignSprintsSprintMark() {
  return (
    <svg
...
export function DesignSprintsWordmark({
  isActive = false,
  centerSrc,
  centerIsCustomUpload = false,
  showCenter = true,
  stickerDots = DEFAULT_STICKER_DOTS,
}) {
  const stickerPool =
    stickerDots.length > 0
      ? stickerDots
      : DEFAULT_STICKER_DOTS
  ...
  const isSessionActive = (session) =>
    isHoveringRef.current && session === hoverSessionRef.current

  const scheduleNext = (delayMs, session = hoverSessionRef.current) => {
    clearTimeoutRef(spawnTimeoutRef)
    spawnTimeoutRef.current = window.setTimeout(() => {
      if (!isSessionActive(session)) return

      const randomDot = stickerPool[randomInt(0, stickerPool.length - 1)]
      ...
      setStickers((prev) => {
        if (!isSessionActive(session)) return prev
      ...
      scheduleNext(nextDelay, session)
    }, delayMs)
  }
            ...
            {showCenter ? (
              centerIsCustomUpload && centerSrc ? (
                <img
                  src={centerSrc}
                  alt=""
                  className="case-thumb__design-sprints-center-img"
                />
              ) : (
                <DesignSprintsSprintMark />
              )
            ) : null}
          </div>
        </div>
